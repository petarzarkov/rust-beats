use super::synthesizer::{get_sample_rate, LowPassFilter, Oscillator, Waveform};
use rand::Rng;
...
pub fn generate_tambourine(velocity: f32) -> Vec<f32> {
    ...
    let mut oscs: Vec<Oscillator> = freqs
        .iter()
...
pub fn generate_cowbell(velocity: f32) -> Vec<f32> {
    ...
    for i in 0..num_samples {
        ...
        let mut sample = osc1.next_sample() * 0.5 + osc2.next_sample() * 0.4;
...
pub fn generate_triangle_perc(velocity: f32) -> Vec<f32> {
    ...
    for i in 0..num_samples {
        ...
        let sample = osc1.next_sample() * 0.5 + osc2.next_sample() * 0.3 + osc3.next_sample() * 0.2;
