...
pub fn generate_rhodes_note(frequency: f32, duration: f32, velocity: f32) -> Vec<f32> {
    ...
    let envelope = Envelope {
        ...
        release: 0.4,     // Long release for warmth
    };
    ...
    // FM modulator for character (slight inharmonicity)
    let mut fm_mod = Oscillator::new(Waveform::Sine, frequency * 14.0);
    
    // Subtle vibrato LFO
    let mut vibrato = LFO::new(4.5, 0.003);
    ...
    for i in 0..num_samples {
        ...
        let fm_amount = fm_mod.next_sample() * 0.3 * env_amp;
