use super::synthesizer::{get_sample_rate, LowPassFilter};
...
pub fn apply_sample_rate_reduction(samples: &mut [f32], target_rate: u32) {
    let reduction_factor = (get_sample_rate() / target_rate) as usize;
...
pub fn apply_wow_flutter(samples: &mut [f32], intensity: f32) {
    ...
    let sample_rate_f = get_sample_rate() as f32;
    ...
    for i in 0..samples.len() {
        ...
        if read_idx < original.len() - 1 {
            ...
            samples[i] = original[read_idx] * (1.0 - frac) + original[read_idx + 1] * frac;
        }
    }
}
...
impl LofiProcessor {
    ...
    pub fn subtle() -> Self {
        LofiProcessor {
            ...
            downsample_rate: get_sample_rate(), // No downsampling
    ...
    pub fn process(&self, samples: &mut [f32]) {
        ...
        if self.downsample_rate < get_sample_rate() {
